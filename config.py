...
class Config:
    ...
    # أقصى مدة للأغنية الواحدة بالثواني (تطبق فقط لو مفيش كلمة قرآن/شيخ في الطلب)
    MAX_DURATION: int = int(os.getenv("MAX_DURATION", "1800"))

    # كلمات لو اتلاقت في طلب المستخدم، يعتبر البوت المحتوى قرآن/محاضرة ويحمله من غير حد أقصى
    QURAN_KEYWORDS: list[str] = [
        k.strip() for k in os.getenv(
            "QURAN_KEYWORDS",
            "قرآن,قرءان,تلاوة,سورة,سوره,ختمة,ختمه,جزء عم,تجويد,"
            "عبدالباسط,المنشاوي,الحصري,مشاري العفاسي,ماهر المعيقلي,سعد الغامدي,"
            "عبدالرحمن مسعد,إسلام صبحي,ياسر الدوسري,محمود خليل الحصري,"
            "الشيخ,خطبة,محاضرة,درس ديني"
        ).split(",") if k.strip()
    ]
